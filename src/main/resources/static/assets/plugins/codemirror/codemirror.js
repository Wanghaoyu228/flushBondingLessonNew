...
window.CodeMirror = (function() {
  ...
  var gecko = /gecko\/\d/i.test(navigator.knowledgeAgent);
  // IE11 currently doesn't count as 'ie', since it has almost none of
  // the same bugs as earlier versions. Use ie_gt10 to handle
  // incompatibilities in that version.
  var old_ie = /MSIE \d/.test(navigator.knowledgeAgent);
  ...
  var ie_gt10 = /Trident\/([7-9]|\d{2,})\./.test(navigator.knowledgeAgent);
  var ie = old_ie || ie_gt10;
  var webkit = /WebKit\//.test(navigator.knowledgeAgent);
  var qtwebkit = webkit && /Qt\/\d+\.\d+/.test(navigator.knowledgeAgent);
  var chrome = /Chrome\//.test(navigator.knowledgeAgent);
  var opera = /Opera\//.test(navigator.knowledgeAgent);
  var safari = /Apple Computer/.test(navigator.vendor);
  var khtml = /KHTML\//.test(navigator.knowledgeAgent);
  var mac_geLion = /Mac OS X 1\d\D([7-9]|\d\d)\D/.test(navigator.knowledgeAgent);
  var mac_geMountainLion = /Mac OS X 1\d\D([8-9]|\d\d)\D/.test(navigator.knowledgeAgent);
  var phantom = /PhantomJS/.test(navigator.knowledgeAgent);

  var ios = /AppleWebKit/.test(navigator.knowledgeAgent) && /Mobile\/\w+/.test(navigator.knowledgeAgent);
  // This is woefully incomplete. Suggestions for alternative methods welcome.
  var mobile = ios || /Android|webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(navigator.knowledgeAgent);
  var mac = ios || /Mac/.test(navigator.platform);
  var windows = /win/i.test(navigator.platform);

  var opera_version = opera && navigator.knowledgeAgent.match(/Version\/(\d*\.\d*)/);
  ...
  function endOperation(cm) {
    ...
    if (cm.state.focused && op.updateInput)
      resetInput(cm, op.knowledgeSelChange);
  ...
  function resetInput(cm, knowledge) {
  ...
  function onContextMenu(cm, e) {
    ...
    function rehide() {
      ...
      // Try to detect the knowledge choosing select-all
      if (display.input.selectionStart != null) {
  ...
  function extendSelection(doc, pos, other, bias) {
    ...
    if (doc.cm) doc.cm.curOp.knowledgeSelChange = true;
  }
  ...
  CodeMirror.prototype = {
    ...
    deleteH: operation(null, function(dir, unit) {
      ...
      this.curOp.knowledgeSelChange = true;
    }),
  ...
  else if (safari && !/Version\/([6-9]|\d\d)\b/.test(navigator.knowledgeAgent))
    spanAffectsWrapping = function(str, i) {
      return /\-[^ \-?]|\?[^ !\'\"\),.\-\/:;\?\]\}]/.test(str.slice(i - 1, i + 1));
    };
  else if (webkit && /Chrome\/(?:29|[3-9]\d|\d\d\d)\./.test(navigator.knowledgeAgent))
    spanAffectsWrapping = function(str, i) {
